...
<form action="{{ route('customer.invitations.update', $invitation->id) }}" method="POST" enctype="multipart/form-data">
    ...
    <div x-data="{ tab: '{{ request()->query('tab') === 'galeri' ? 'galeri' : 'mempelai' }}' }" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="flex gap-1 border-b border-zinc-200 dark:border-zinc-700 overflow-x-auto">
                @foreach (['mempelai' => 'Data Mempelai', 'acara' => 'Rangkaian Acara', 'galeri' => 'Galeri Foto/Video', 'pengaturan' => 'Pengaturan (BGM & Amplop)'] as $key => $label)
                    <button
                        type="button"
                        @click="tab = '{{ $key }}'"
                        :class="tab === '{{ $key }}' ? 'border-amber-600 text-amber-600 dark:text-amber-500 font-bold' : 'border-transparent text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 font-medium'"
                        class="px-4 py-2 text-sm border-b-2 whitespace-nowrap transition-colors"
                    >{{ $label }}</button>
                @endforeach
            </div>
            ...
            <div>
                ...
                <div x-show="tab === 'pengaturan'" x-cloak class="space-y-6">
                    ...
                    <flux:card class="flex flex-col gap-6">
                        ...
                        @else
                            <div class="p-4 rounded-xl bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-800 text-sm">
