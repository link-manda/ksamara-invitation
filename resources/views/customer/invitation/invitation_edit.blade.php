    ...
    <div x-data="{ tab: '{{ $errors->hasAny(['events', 'events.*']) ? 'acara' : ($errors->hasAny(['galleries', 'galleries.*']) ? 'galeri' : ($errors->hasAny(['music_path', 'envelopes', 'envelopes.*']) ? 'pengaturan' : (request()->query('tab') === 'galeri' ? 'galeri' : 'mempelai'))) }}' }" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-6">
            ...
            <flux:card
                x-data="{
                    names: [''],
                    baseUrl: '{{ url('/') }}/{{ $invitation->slug }}',
                    copyDone: null,
                    addName() { this.names.push(''); },
                    removeName(i) { this.names.splice(i, 1); },
                    getUrl(name) {
                        return name.trim()
                            ? this.baseUrl + '?to=' + encodeURIComponent(name.trim())
                            : this.baseUrl;
                    },
                    copy(name, i) {
                        navigator.clipboard.writeText(this.getUrl(name));
                        this.copyDone = i;
                        setTimeout(() => this.copyDone = null, 2000);
                    }
                }"
                class="space-y-4"
            >
                <flux:heading size="sm">Bagikan Undangan</flux:heading>
                <p class="text-xs text-zinc-500">Generate link personal per tamu — nama otomatis terisi saat dibuka.</p>
                        ...
                        <div class="space-y-1.5">
                            <div class="flex gap-2">
                                <input
                                    ...
                                    class="flex-1 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-3 py-2 text-xs text-zinc-900 dark:text-white focus:border-amber-400 focus:ring-1 focus:ring-amber-400 outline-none"
                                />
                                <button
                                    type="button"
                                    @click="removeName(i)"
                                    x-show="names.length > 1"
                                    class="p-2 rounded-lg text-zinc-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950 transition-colors"
                                >
                            ...
                            <button
                                type="button"
                                @click="copy(names[i], i)"
                                :disabled="!names[i].trim()"
                                class="w-full inline-flex items-center justify-center gap-1.5 py-1.5 rounded-lg border border-zinc-200 dark:border-zinc-700 text-[11px] font-semibold text-zinc-600 dark:text-zinc-300 hover:border-amber-400 hover:text-amber-700 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                            >
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/>
                                </svg>
                                <span x-text="copyDone === i ? 'Link Tersalin!' : 'Salin Link'"></span>
                            </button>
                        </div>
                    </template>
                </div>
                ...
                <button
                    type="button"
                    @click="addName()"
                    class="w-full py-2 rounded-lg border border-dashed border-zinc-300 dark:border-zinc-600 text-xs text-zinc-500 hover:border-amber-400 hover:text-amber-600 transition-colors"
                >
                    + Tambah Nama Tamu
                </button>
            </flux:card>
